...
function downloadFile(sourceID) {
	...
	var str = "servlet/DownloadSDServlet?param1=" + fd;
	...
	if (contractCodeAll != "") {
		...
		$.ajax({
			...
			success : function(response) {
				...
				// description map.
				//updateContractMap();	//Disabled as on 29-03-2017.
			},
			error : function(e) {
				alert('Error: ' + e);
			}
...
function updateContractMap(sourceID, destID, value) {
	...
	if (http != null) {
		...
		http.open("GET", "securityDeposit.htm?_target3=securityDeposit&unique=" + random
