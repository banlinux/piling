...
function findSch() {
	...
	getSelSchDesc(e.options[e.selectedIndex].value);
}
...
function downloadFile(sourceID) {
	...
	if (fd == null || fd == "") {
		alert("Please wait for file to be generated");
	...
	var str = "servlet/DownloadISServlet?param1=" + fd;
...
// Step-4: Obtain toDate field to evaluate current position of contract within
// selected zone.
function zoneForward() {
	...
	var fromDate = document.getElementById("fromDate").value; // contains
	// fromDate
	// value.
	var toDate = document.getElementById("toDate").value; // contains toDate
	// value.
	...
	if (len1 > 0) {
		...
		for (var i = 0; i < len1; i++) {
			...
			zoneCodeAll += "'" + e1.options[i].value + "',"; // include
			// already
			// selected
			// zones.
		}
	...
	$.ajax({
		type : "POST",
		url : "incomeSchedule.htm",
		data : "zoneCode=" + zoneCodeAll + "&fromDate=" + fromDate + "&toDate="
				+ toDate + "&_target11=contract",
		success : function(response) {
			var data = $.parseJSON(response);
			...
			// zone.
			/*
			 * var len3 = $("#contract option").length; var html = ''; //
			 * display of contract code instead of contract description in the //
			 * map. if (len3 > 0) { var e3 =
			 * document.getElementById("contract");
			 * 
			 * for (var i = 0; i < len3; i++) { html += '<option value="' +
			 * e3.options[i].value + '">' + e3.options[i].value + '</option>'; } }
			 */
...
function zoneForwardAll() {
	...
	//alert("Total zones selected: "+len);
	...
	$.ajax({
		...
		data : "zoneCode=" + zoneCodeAll + "&fromDate=" + fromDate + "&toDate="
				+ toDate + "&_target11=contract",
		// data : "zoneCode=" + zoneCodeAll + "&toDate=" + toDate
		// +"&_target11=contract",
		success : function(response) {
			var data = $.parseJSON(response);
...
function zoneReverse() {
	...
	if (zoneCodeAll != "") {
		...
		$('#contract_desc').html(''); // Free contract_desc map.
	}
...
function updateContractMap(sourceID, destID, value) {
	...
	if (http != null) {
		...
		http.open("GET", "incomeSchedule.htm?_target3=incomeSchedule&unique=" + random
...
function contractMoveAll(sourceID, destID) {
	...
	// ------ Populate Contract Description Map of selected contract.
	populateContract(sourceID, destID, src, dest);
	// $('#contract_desc').html(''); //To blank the description.

	// var e = document.getElementById("selected_contract"); // contains
	// selected
	// var len1 = $("#selected_contract").length-1;
...
// Added on 09-03-2017 prior to final upload in LIVE.
